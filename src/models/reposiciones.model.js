...
export const createReposicion = async (data) => {
  const { clave_medicamento, cantidad, fecha_reposicion, notas } = data;

  // 1. Insertar la reposición
  const [result] = await db.query(
    `INSERT INTO reposiciones (clave_medicamento, cantidad, fecha_reposicion, notas)
     VALUES (?, ?, ?, ?)`,
    [clave_medicamento, cantidad, fecha_reposicion, notas || null]
  );

  // 2. Sumar al stock_inicial del medicamento (sobrante + nuevo lote)
  await db.query(
